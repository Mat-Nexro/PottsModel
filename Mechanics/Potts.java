...
public void PottsSim() {
		...
		double h1 = -1*((xp ? 1 : 0) + (xl ? 1 : 0) + (yg ? 1 : 0) + (yd ? 1 : 0));
		int ns;
		...
		double h2 = -1*((xp2 ? 1 : 0) + (xl2 ? 1 : 0) + (yg2 ? 1 : 0) + (yd2 ? 1 : 0));
	...
	public synchronized void run() {
		...
		while(true) {
			...
			if(l == N) {
					...
					if(time==1) {
						try {
							outM = new FileWriter("M"+"q"+q+"_T"+T+"_"+"P"+np+".txt",true);
							outE = new FileWriter("E"+"q"+q+"_T"+T+"_"+"P"+np+".txt",true);
							outA = new FileWriter("A"+"q"+q+"_T"+T+"_"+"P"+np+".txt",true);
						} catch (IOException e) {}
					}
